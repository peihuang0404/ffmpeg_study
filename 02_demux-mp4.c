...
int main(int argc, char **argv) {
  ...
  char errors[ERROR_STRING_SIZE +
              1]; // 主要是用来缓存解析FFmpeg api返回值的错误string
  ...
  const AVBitStreamFilter *bsfilter =
      av_bsf_get_by_name("h264_mp4toannexb"); // 对应面向对象的方法
  if (!bsfilter) {
    avformat_close_input(&ifmt_ctx);
  ...
  while (1) {
    ret = av_read_frame(
        ifmt_ctx,
        pkt); // 不会去释放pkt的buf，如果我们外部不去释放，就会出现内存泄露
    if (ret < 0) {
      av_strerror(ret, errors, ERROR_STRING_SIZE);
    ...
    if (pkt->stream_index == video_index) {
      // 处理视频
      ret = av_bsf_send_packet(bsf_ctx,
                               pkt); // 内部把我们传入的buf转移到自己bsf内部
      if (ret < 0) {                 // 基本不会进入该逻辑
        av_strerror(ret, errors, ERROR_STRING_SIZE);
      ...
      while (1) {
        ...
        if (size != pkt->size) {
          av_log(NULL, AV_LOG_DEBUG,
                 "h264 warning, length of writed data isn't equal "
                 "pkt->size(%ld, %d)\n",
                 size, pkt->size);
        }
        av_packet_unref(pkt);
      ...
      adts_header(
          adts_header_buf, pkt->size,
          ifmt_ctx->streams[audio_index]->codecpar->profile,
          ifmt_ctx->streams[audio_index]->codecpar->sample_rate,
          ifmt_ctx->streams[audio_index]->codecpar->ch_layout.nb_channels);
      fwrite(adts_header_buf, 1, 7,
             aac_fd); // 写adts header ,
                      // ts流不适用，ts流分离出来的packet带了adts header
      size_t size = fwrite(pkt->data, 1, pkt->size, aac_fd); // 写adts data
      if (size != pkt->size) {
        av_log(NULL, AV_LOG_DEBUG,
               "aac warning, length of writed data isn't equal pkt->size(%ld, "
               "%d)\n",
               size, pkt->size);
      }
      av_packet_unref(pkt);

...
static int check_channel_layout(const AVCodec *codec,
                                const uint64_t channel_layout) {
  // 不是每个codec都给出支持的channel_layout
  const int *p = &codec->ch_layouts->nb_channels;
  if (!p) {
...
static int encode(AVCodecContext *ctx, AVFrame *frame, AVPacket *pkt,
  ...
  while (ret >= 0) {
    ...
    printf("ctx->flags:0x%x & AV_CODEC_FLAG_GLOBAL_HEADER:0x%x, name:%s\n",
           ctx->flags, ctx->flags & AV_CODEC_FLAG_GLOBAL_HEADER,
           ctx->codec->name);
...
/*
 * 提取测试文件：
 * （1）s16格式：ffmpeg -i buweishui.aac -ar 48000 -ac 2 -f s16le
 * 48000_2_s16le.pcm （2）flt格式：ffmpeg -i buweishui.aac -ar 48000 -ac 2 -f
 * f32le 48000_2_f32le.pcm
 *      ffmpeg只能提取packed格式的PCM数据，在编码时候如果输入要为fltp则需要进行转换
 * 测试范例:
 * （1）48000_2_s16le.pcm libfdk_aac.aac libfdk_aac  //
 * 如果编译的时候没有支持fdk aac则提示找不到编码器 （2）48000_2_f32le.pcm
 * aac.aac aac // 我们这里只测试aac编码器，不测试fdkaac
 */
int main(int argc, char **argv) {
  ...
  if (force_codec == 0) { // 没有强制设置编码器
    codec =
        avcodec_find_encoder(codec_id); // 按ID查找则缺省的aac encode为aacenc.c
  } else {
  ...
  if (!check_channel_layout(codec, codec_ctx->ch_layout.nb_channels)) {
    fprintf(stderr, "Encoder does not support channel layout %d",
  ...
  codec_ctx->flags =
      AV_CODEC_FLAG_GLOBAL_HEADER; // ffmpeg默认的aac是不带adts，而fdk_aac默认带adts，这里我们强制不带
  /* 将编码器上下文和编码器进行关联 */
  ...
  for (;;) {
    ...
    */
    ...
    if (AV_SAMPLE_FMT_S16 == frame->format) {
      // 将读取到的PCM数据填充到frame去，但要注意格式的匹配,
      // 是planar还是packed都要区分清楚
      ret = av_samples_fill_arrays(frame->data, frame->linesize, pcm_buf,
                                   frame->ch_layout.nb_channels,
      ...
      f32le_convert_to_fltp((float *)pcm_buf, (float *)pcm_temp_buf,
                            frame->nb_samples);

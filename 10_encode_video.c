...
static int encode(AVCodecContext *enc_ctx, AVFrame *frame, AVPacket *pkt,
  ...
  while (ret >= 0) {
    ...
    if (pkt->flags & AV_PKT_FLAG_KEY)
      printf("Write packet flags:%d pts:%3" PRId64 " dts:%3" PRId64
             " (size:%5d)\n",
...
/**
 * @brief 提取测试文件：ffmpeg -i test_1280x720.flv -t 5 -r 25 -pix_fmt yuv420p
 * yuv420p_1280x720.yuv 参数输入: yuv420p_1280x720.yuv yuv420p_1280x720.h264
 * libx264
 * @param argc
 * @param argv
 * @return
...
int main(int argc, char **argv) {
  ...
  /* 设置I帧间隔
   * 如果frame->pict_type设置为AV_PICTURE_TYPE_I,
   * 则忽略gop_size的设置，一直当做I帧进行编码
   */
  ...
  if (codec->id == AV_CODEC_ID_H264) {
    ...
    ret = av_opt_set(codec_ctx->priv_data, "tune", "zerolatency",
                     0); // 直播是才使用该设置
  ...
  //    codec_ctx->thread_count = 4;  // 开了多线程后也会导致帧输出延迟,
  //    需要缓存thread_count帧后再编程。 codec_ctx->thread_type =
  //    FF_THREAD_FRAME; // 并 设置为FF_THREAD_FRAME
  /* 对于H264 AV_CODEC_FLAG_GLOBAL_HEADER  设置则只包含I帧，此时sps
   * pps需要从codec_ctx->extradata读取 不设置则每个I帧都带 sps pps sei
   */
  ...
  for (;;) {
    ...
    */
    ...
    if (av_frame_is_writable(frame) == 0) { // 这里只是用来测试
      ...
      if (frame->buf &&
          frame->buf[0]) // 打印referenc-counted，必须保证传入的是有效指针

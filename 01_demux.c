...
int main(int argc, char **argv) {
  ...
  total_seconds =
      (ifmt_ctx->duration) / AV_TIME_BASE; // 1000us = 1ms, 1000ms = 1秒
  ...
  for (uint32_t i = 0; i < ifmt_ctx->nb_streams; i++) {
    ...
    if (AVMEDIA_TYPE_AUDIO == in_stream->codecpar->codec_type) {
      ...
      if (in_stream->duration != AV_NOPTS_VALUE) {
        int duration_audio =
            (in_stream->duration) * av_q2d(in_stream->time_base);
                   ...
                   ->codec_type) // 如果是视频流，则打印视频的信息
    {
      ...
      if (AV_CODEC_ID_MPEG4 ==
          in_stream->codecpar->codec_id) // 视频压缩编码格式
      {
    ...
    if (pkt_count++ < print_max_count) {
      if (pkt->stream_index == audioindex) {
        ...
        printf("audio duration: %lf\n\n",
               pkt->duration *
                   av_q2d(ifmt_ctx->streams[audioindex]->time_base));
        ...
        printf("video duration: %lf\n\n",
               pkt->duration *
                   av_q2d(ifmt_ctx->streams[videoindex]->time_base));
      } else {
